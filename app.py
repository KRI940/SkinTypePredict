...
BASE = r"K:\Kritika\Internship Project\AI Skin type Predictor\SkinTypePredict\ingredients"
...
def open_landing_page():
    ...
    # --- Logo ---
    try:
        logo_img = Image.open("logo1.png").resize((90, 90), Image.LANCZOS)
    ...
    def open_main():
        if not users_exist():
            ...
            try:
                reg_logo_img = Image.open("logo1.png").resize((150, 150), Image.LANCZOS)
    ...
    try:
        bg_image_orig = Image.open("img3.JPG")
    except:
        bg_image_orig = None
...
def open_main_app(parent):
    ...
    encoders = {}
    try:
        os.chdir(r"K:\Kritika\Internship Project\AI Skin type Predictor\SkinTypePredict")
    ...
    def predict_skin_type():
        ...
        try:
            ...
            result_label.config(text=f"Error: {e}", fg=turquoise)
    ...
    def show_recommendations():
        ...
        back_btn = tk.Button(rec_win, text="← Back", bg=teal, fg="white",font=("Arial", 10, "bold"), relief="flat",command=rec_win.destroy)
        ...
        for idx, ingredient in enumerate(ingredients):
            ...
            tk.Label(card, text=ingredient, font=("Arial", 11, "bold"),
                     bg="white", fg=darkblue).pack(pady=(0, 6))
    ...
    # Background
    try:
        bg_image_main = Image.open("img2.jpg")
    except Exception:
        bg_image_main = None
    ...
    fields = {
        "Age": tk.Entry(form_frame_main, bg=GRAY90, fg=darkblue, relief="flat", font=entry_font, width=20),
        "Gender": ttk.Combobox(form_frame_main, values=encoders["gender"].classes_.tolist(), font=entry_font, width=18, foreground=darkblue),
        "Water Intake (liters)": tk.Entry(form_frame_main, bg=GRAY90, fg=darkblue, relief="flat", font=entry_font, width=20),
        "Weather": ttk.Combobox(form_frame_main, values=encoders["weather"].classes_.tolist(), font=entry_font, width=18, foreground=darkblue),
        "Oiliness": ttk.Combobox(form_frame_main, values=encoders["oiliness"].classes_.tolist(), font=entry_font, width=18, foreground=darkblue),
        "Acne": ttk.Combobox(form_frame_main, values=encoders["acne"].classes_.tolist(), font=entry_font, width=18, foreground=darkblue),
        "Tightness After Wash": ttk.Combobox(form_frame_main, values=encoders["tightness_after_wash"].classes_.tolist(), font=entry_font, width=18, foreground=darkblue),
        "Makeup Usage": ttk.Combobox(form_frame_main, values=encoders["makeup_usage"].classes_.tolist(), font=entry_font, width=18, foreground=darkblue),
        "Flaking": ttk.Combobox(form_frame_main, values=encoders["flaking"].classes_.tolist(), font=entry_font, width=18, foreground=darkblue),
        "Redness/Itchiness": ttk.Combobox(form_frame_main, values=encoders["redness_itchiness"].classes_.tolist(), font=entry_font, width=18, foreground=darkblue)
    }
    ...
    for i, (label_text, widget) in enumerate(items):
        ...
        tk.Label(form_frame_main, text=label_text, bg="#ffffff", fg=teal, font=label_font)\
    ...
    result_label = tk.Label(form_frame_main, text="", font=("Arial", 14, "bold"),
                            fg=turquoise, bg="#ffffff", justify="left")
    result_label.grid(row=total_rows, column=0, columnspan=col_count, pady=10)

    predict_button = tk.Button(form_frame_main, text="Predict", bg=teal, fg="white",
    ...
    recommend_button = tk.Button(form_frame_main, text="Show Recommendations", bg=teal, fg="white",
    ...
    recommendation_label = tk.Label(form_frame_main, text="", font=("Arial", 12 , "bold"),
                                     fg=teal, bg="#ffffff", justify="left")
    recommendation_label.grid(row=total_rows+3, column=0, columnspan=col_count, pady=10)

    back_button = tk.Button(main_win, text="← Back", bg="#2F4F4F", fg="white",
...
def create_new_account(parent, logo_photo):
    ...
    # Back Button
    back_button = tk.Button(form_box, text="← Back", bg=teal, fg="white",
                            font=("Arial", 10, "bold"), relief="flat",
                            command=lambda: (reg_win.destroy(), parent.deiconify()))
    back_button.pack(anchor="w", pady=(0, 15))
    add_hover_effect(back_button, teal)

    tk.Label(form_box, text="Create a SkinSaathi Account", font=("Arial", 18, "bold"), bg="#ffffff", fg=teal)\
        .pack(pady=(0, 15))

    tk.Label(form_box, text="Full Name", bg="#ffffff", font=unified_font, fg=teal).pack(anchor="w")
    full_name = tk.Entry(form_box, bg=GRAY90, fg=darkblue, relief="flat", font=unified_font)
    full_name.pack(pady=5, fill="x")
    add_hover_effect(full_name, GRAY90)

    tk.Label(form_box, text="Username", bg="#ffffff", font=unified_font, fg=teal).pack(anchor="w")
    new_username = tk.Entry(form_box, bg=GRAY90, fg=darkblue, relief="flat", font=unified_font)
    new_username.pack(pady=5, fill="x")
    add_hover_effect(new_username, GRAY90)

    tk.Label(form_box, text="Password", bg="#ffffff", font=unified_font, fg=teal).pack(anchor="w")
    new_password = tk.Entry(form_box, show="*", bg=GRAY90, fg=darkblue, relief="flat", font=unified_font)
    new_password.pack(pady=5, fill="x")
    add_hover_effect(new_password, GRAY90)

    tk.Label(form_box, text="Confirm Password", bg="#ffffff", font=unified_font, fg=teal).pack(anchor="w")
    confirm_password = tk.Entry(form_box, show="*", bg=GRAY90, fg=darkblue, relief="flat", font=unified_font)
    ...
    create_btn = tk.Button(form_box, text="Create Account", command=save_user,
                           bg=teal, fg="white", font=("Arial", 12, "bold"), relief="flat")
    create_btn.pack(pady=15)
    add_hover_effect(create_btn, teal)
...
def build_login_window():
    ...
    try:
        logo_img = Image.open(r"K:\Kritika\Internship Project\AI Skin type Predictor\SkinTypePredict\logo1.png").resize((200, 200), Image.LANCZOS)
        logo_photo = ImageTk.PhotoImage(logo_img)
    except Exception as e:
        logo_photo = None
    ...
    try:
        bg_image_login = Image.open("img2.jpg")
    except:
        bg_image_login = None
    ...
    tk.Label(form_box, text="Login", font=("Arial", 22, "bold"), bg="#ffffff", fg=teal).pack(pady=(18, 8))

    tk.Label(form_box, text="Username or Email Address", bg="#ffffff", font=unified_font, fg=teal).pack(anchor="w")
    username_entry = tk.Entry(form_box, bg=GRAY90, fg=darkblue, relief="flat", font=unified_font)
    username_entry.pack(pady=5, fill="x")
    add_hover_effect(username_entry, GRAY90)

    tk.Label(form_box, text="Password", bg="#ffffff", font=unified_font, fg=teal).pack(anchor="w")
    password_entry = tk.Entry(form_box, show="*", bg=GRAY90, fg=darkblue, relief="flat", font=unified_font)
    ...
    login_btn = tk.Button(form_box, text="Log In", bg=teal, fg="white", font=("Arial", 12, "bold"), relief="flat",
                          command=check_login)
    login_btn.pack(pady=(12, 10))
    add_hover_effect(login_btn, teal)
